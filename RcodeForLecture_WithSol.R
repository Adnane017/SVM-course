...
######################## Task 1 ########################### 
# Model fit by inputing feature matrix and response vector 
...
# 
# ##################################################
# # Case 4: Soft margin classifier with non-linearly 
# #         separable classes 
# ##################################################
# 
# ### Preparing the dataset
# # We add some points on the wrong side of the previous margin
# ExtraPoints.dat = data.frame(x.1 = c(1,1.8), 
#                              x.2 = c(-0.5,-0.5),
#                              y = as.factor(c("1","-1"))) 
# dat2 = rbind(dat,ExtraPoints.dat)
# 
# ### Ploting the data and checking that there are now two points 
# ### on the wrong side of the margin
# plot(dat2$x.1, dat2$x.2, col = 3+as.numeric(as.character(dat2$y)), 
#      pch = 19, xlab = "X1", ylab = "X2")
# grid()
# legend(-1.7, 3, pch=c(19,19), legend=c("Y = -1", "Y = 1"), 
#        bty="o", col=c(2,4), box.col="black", cex=.8)
# 
# ### Fitting a support vector classifier using a smaller cost (=0.2), 
# ### which is equivalent to having a large C value (thus the classifier
# ### would tolarate classification errors)
# svmfit.02 = svm(y~., data = dat, kernel = "linear", cost = 0.2, scale = FALSE) 
# svmfit.5 = svm(y~., data = dat, kernel = "linear", cost = 5, scale = FALSE) 
# 
# # Generate the plot for the new SV classifier and put it along side
# # with the previous plot
# par(mfrow=c(1,2))
# # New plot
# plot_SVM(dat2, svmfit.02, 49)
# title(main = "cost = 0.2 (i.e. C = 5)")
# # Previous plot
# plot_SVM(dat2, svmfit.5, 49)
# title(main = "cost = 5 (i.e. C = 0.2)")
# par(mfrow=c(1,1))
# 
# #########################################################
# 
